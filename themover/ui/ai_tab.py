...
class AITab(QWidget):
    ...
    def _client_or_warn(self, modal: bool = True) -> Optional[ClaudeClient]:
        if not self.ctx.settings.effective_api_key:
            if modal:
                ...
                self._append_system("Paste your Anthropic API key in the Setup tab first, then send the recording again.")
            return None
    ...
    def send_recording(self, session, explanation: str, complete: bool) -> None:
        ...
        def job(signals):
            ...
            return chat.send(content, on_text=lambda t: signals.text.emit(t))
