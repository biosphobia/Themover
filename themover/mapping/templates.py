...
TEMPLATES: dict[str, dict] = {
    ...
    "osu_taiko": {
        ...
        "description": "Each controller is a drumstick. Strike down for don (centre), swing outward for kat (rim). Default osu! keys: D K = kat, F J = don.",
        "play_style": "Hold a controller in each hand like drumsticks pointing at the screen. Hit downward with the right hand for a right don (J) and with the left hand for a left don (F). Swing outward to the side for kat: right hand to the right (K), left hand to the left (D). Big notes: strike with both hands. The trigger and Move button also work as don/kat if your arms get tired. Cross = Enter, Circle = Escape, Square/Triangle scroll the song list.",
        "controllers": [{"color": [255, 40, 40], "role": "right stick"}, {"color": [40, 120, 255], "role": "left stick"}],
        "bindings": [
            _b("c0.gesture.swing_down", "key.j", mode="tap", tap_ms=35, comment="right don (strike down)"),
            _b("c0.gesture.swing_up", "key.j", mode="tap", tap_ms=35, comment="right don (the stop of the strike)"),
            _b("c1.gesture.swing_down", "key.f", mode="tap", tap_ms=35, comment="left don"),
            _b("c1.gesture.swing_up", "key.f", mode="tap", tap_ms=35, comment="left don"),
            _b("c0.gesture.swing_right", "key.k", mode="tap", tap_ms=35, comment="right kat (swing outward)"),
            _b("c1.gesture.swing_left", "key.d", mode="tap", tap_ms=35, comment="left kat (swing outward)"),
            _b("c0.trigger", "key.j", threshold=0.5, comment="button fallback: right don"),
            _b("c0.button.move", "key.k", comment="button fallback: right kat"),
            _b("c1.trigger", "key.f", threshold=0.5, comment="button fallback: left don"),
            _b("c1.button.move", "key.d", comment="button fallback: left kat"),
            _b("c0.button.cross", "key.enter", mode="tap", comment="select"),
            _b("c0.button.circle", "key.esc", mode="tap", comment="back / pause"),
            _b("c0.button.triangle", "key.up", mode="repeat", repeat_ms=120, comment="song list up"),
            _b("c0.button.square", "key.down", mode="repeat", repeat_ms=120, comment="song list down"),
            _b("c1.button.triangle", "key.f2", mode="tap", comment="random song"),
            _b("c1.button.square", "key.grave", mode="tap", comment="quick retry"),
            _b("c0.button.start", "key.esc", mode="tap"),
        ],
        "feedback": [
            _fb(when="c0.gesture.swing_any", controller=0, rumble=1.0, duration_ms=45, led=[255, 255, 255], led_duration_ms=45, threshold=0.5, comment="drum thump"),
            _fb(when="c1.gesture.swing_any", controller=1, rumble=1.0, duration_ms=45, led=[255, 255, 255], led_duration_ms=45, threshold=0.5, comment="drum thump"),
        ],
